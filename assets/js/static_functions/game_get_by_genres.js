...
async function fetchGameGetByGenres() {
    ...
    try {
        ...
        const apiUrl = `http://localhost:5000/api/gamebygenres/${genre}`;
        ...
        if (response.ok) {
            ...
            document.querySelector(".catalog_page_1_list_genres").textContent = data.genre;
            ...
            for (let item in items) {
                ...
                document.querySelector('.catalog_page_1_list_2').innerHTML += data_for_print;
            }
            //check_like();
            check_purchase();
        } else {
