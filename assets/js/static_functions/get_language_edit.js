        ...
        if (response.ok) {
            ...
            for (let item in items) {
                const data_for_print = `
                    <div class="game_stats_language_to_update_label">
                        <input id="${items[item].toLowerCase()}" class="game_stats_language_to_update" type="checkbox" placeholder="${items[item]}" name="${items[item].toLowerCase()}" value="${items[item]}">
                        <label class="checkbox_style" for="${items[item].toLowerCase()}">${items[item]}</label>
                    </div>`;
                document.querySelector('.game_stats_language_to_update').innerHTML += data_for_print;
            }
        } else {
