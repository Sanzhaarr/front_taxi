...
registrationForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        const apiUrl = "http://localhost:5000/api/reg/driver/registration";
...
loginForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        const apiUrl = "http://localhost:5000/api/reg/driver/login";

        const response = await fetch(apiUrl, requestOptions);
        const data = await response.json();

        //document.cookie = `token=${data.token}; expires=Thu, 17 May 2024 00:00:00 UTC; path=/`;
        localStorage.setItem('token', data.token_driver);

        if (response.ok) {
            console.log(data.message);
            console.log(data.token_driver);
